...
def get_CFG_reader(cfg_path: Optional[str]=None):
    ...
    assert pkg_resources.resource_exists('MiCoGPT', 'resources/mgm_config.ini')
    cfg.read(pkg_resources.resource_filename('MiCoGPT', 'resources/mgm_config.ini'))
    return cfg


def find_pkg_resource(path):
    if pkg_resources.resource_exists('MiCoGPT', path):
        return pkg_resources.resource_filename('MiCoGPT', path)
    else:
