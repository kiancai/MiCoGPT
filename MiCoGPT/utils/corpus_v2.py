...
class MiCoGPTCorpus(Dataset):
    def __init__(self, 
        ...
        # [v2] 如果需要使用 metadata，检查是否提供了 metadata
        ...
        for sample in tqdm(self.data.index):
            # [v2] 修改调用：传入 sample_id (即 sample)
            tokens, length = self._convert_to_token(self.data.loc[sample], sample_id=sample)
    ...
    def _convert_to_token(self, sample, sample_id=None):
        # 删除 z-score 低于 0 的 OTU，然后降序排序
        sample = sample[sample > self.zero_values]
        sample = sample.sort_values(ascending=False)

        # [v2] 构建 Metadata Token 序列
        meta_tokens = []
        if self.use_meta_cols and sample_id is not None:
            # 获取该样本的 metadata 行
            # 注意：self.metadata 已经按 sample_ids 对齐，但为了安全直接用 loc
            ...
            for col in self.use_meta_cols:
                ...
                if pd.isna(val) or str(val).strip() == "" or str(val).lower() == "nan":
                    token_str = f"meta__{col}:<NA>"
                else:
                    token_str = f"meta__{col}:{str(val).strip()}"
                
                meta_tokens.append(token_str)

        # [v2] 将 Metadata tokens 加入序列
        # 结构: <bos> + [Meta1, Meta2...] + [Taxon1, Taxon2...] + <eos>
        sent = ['<bos>'] + meta_tokens + sample.index.tolist() + ['<eos>']
